...
class VrchatFriendRepository {
    ...
    async executeDelete(path, params = null, { endpoint = '' } = {}) {
        return this.execute(path, { endpoint, method: 'DELETE', params });
    }

    async executePost(path, params = null, { endpoint = '' } = {}) {
        return this.execute(path, { endpoint, method: 'POST', params });
    }

    async getFriends({ endpoint = '', offline = false, n = PAGE_SIZE, offset = 0 } = {}) {
    ...
    async getAllFriends({ endpoint = '', offline = false } = {}) {
        const friends = [];

        for (let offset = 0; offset <= MAX_OFFSET; offset += PAGE_SIZE) {
            const response = await this.getFriends({
                endpoint,
                offline,
                n: PAGE_SIZE,
                offset
    ...
    async getUser({ userId, endpoint = '' }) {
        ...
        return this.executeGet(`users/${encodeURIComponent(normalizedUserId)}`, {}, { endpoint });
    }

    async deleteFriend({ userId, endpoint = '' }) {
        const normalizedUserId =
            typeof userId === 'string' ? userId.trim() : String(userId ?? '').trim();
        if (!normalizedUserId) {
    ...
    async sendFriendRequest({ userId, endpoint = '' }) {
        const normalizedUserId =
            typeof userId === 'string' ? userId.trim() : String(userId ?? '').trim();
        if (!normalizedUserId) {
    ...
    async cancelFriendRequest({ userId, notificationId = '', endpoint = '' }) {
        const normalizedUserId =
            typeof userId === 'string' ? userId.trim() : String(userId ?? '').trim();
        if (!normalizedUserId) {
...
const vrchatFriendRepository = new VrchatFriendRepository();

export { VrchatFriendRepository };
