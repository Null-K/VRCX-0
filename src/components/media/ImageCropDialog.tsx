...
const DEFAULT_VIEW: View = { offsetX:0, offsetY:0, scale:1, flipH:false, flipV:false };
...
function clampOffset(
    ...
    const sl = s.x - cw/2,  sr = s.x + s.w - cw/2;
    const st = s.y - ch/2,  sb = s.y + s.h - ch/2;
    return {
...
async function renderCroppedBlob(
    ...
    ctx.scale(view.flipH ? -1 : 1, view.flipV ? -1 : 1);
    ctx.drawImage(img, srcX, srcY, outW, outH, -outW/2, -outH/2, outW, outH);
    ctx.restore();
...
export function ImageCropDialog({
    open, title, description, file, aspectRatio = 1,
    noteField, cropWhiteBorderField, onOpenChange, onConfirm
}: any) {
    ...
    const resolvedDescription = description || t('message.image.description.crop_description');
    ...
    const cropWhiteBorderDefault = cropWhiteBorderField?.defaultChecked !== false;
    ...
    useEffect(() => {
        ...
        img.src = url;
        return () => { URL.revokeObjectURL(url); imgRef.current = null; };
    }, [file, open, setView]);

    useEffect(() => {
        setNote('');
        setCropWhiteBorder(cropWhiteBorderDefault);
    }, [cropWhiteBorderDefault, cropWhiteBorderEnabled, file, noteEnabled, open]);
    ...
    const onPointerDown = useCallback((e: React.PointerEvent<HTMLCanvasElement>) => {
        ...
        dragRef.current = { sx: e.clientX, sy: e.clientY, ox: viewRef.current.offsetX, oy: viewRef.current.offsetY };
    }, []);
    ...
    const onWheel = useCallback((e: React.WheelEvent<HTMLCanvasElement>) => {
        ...
        setView(prev => {
            const nextScale = Math.min(SCALE_MAX, Math.max(SCALE_MIN, prev.scale * factor));
            ...
            return { ...prev, scale: nextScale, ...clampOffset(rawOx, rawOy, rw, rh, s, cw, ch) };
        });
    }, [aspect, setView]);
    ...
    const applyZoom = useCallback((factor: number) => {
        ...
        setView(prev => {
            ...
            const rw = img.naturalWidth  * base * nextScale;
            const rh = img.naturalHeight * base * nextScale;
            return { ...prev, scale: nextScale, ...clampOffset(prev.offsetX, prev.offsetY, rw, rh, s, cw, ch) };
        });
    }, [aspect, setView]);

    const flipHorizontal = useCallback(() => setView(p => ({ ...p, flipH: !p.flipH })), [setView]);
    const flipVertical   = useCallback(() => setView(p => ({ ...p, flipV: !p.flipV })), [setView]);
    ...
    async function confirmCrop() {
        if (!file || !validateImageUploadFile(file).ok || !imgRef.current) return;
        const canvas = canvasRef.current;
        if (!canvas) return;
        setIsConfirming(true);
        try {
            const blob = await renderCroppedBlob(
                imgRef.current, viewRef.current, aspect,
            ...
            await onConfirm?.(blob, Object.keys(opts).length > 0 ? opts : undefined);
        } finally {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-3xl">
                ...
                <div className="flex flex-col gap-4">
                    <div className="bg-muted overflow-hidden rounded-lg border">
                        <canvas
                            ref={canvasRef}
                            role="img"
                            aria-label={t('message.image.success.selected_upload_preview')}
                    ...
                    <FieldGroup>
                        <div
                            ...
                            role="toolbar"
                            aria-label={t('dialog.image_crop.toolbar_label', { defaultValue: 'Image crop toolbar' })}
                        >
                            <Button variant="outline" size="icon" onClick={flipHorizontal} disabled={!imgReady}
                                title={t('dialog.image_crop.flip_h')} aria-label={t('dialog.image_crop.flip_h')}>
                                <FlipHorizontal2 className="h-4 w-4" />
                            </Button>
                            <Button variant="outline" size="icon" onClick={flipVertical} disabled={!imgReady}
                                title={t('dialog.image_crop.flip_v')} aria-label={t('dialog.image_crop.flip_v')}>
                                <FlipVertical2 className="h-4 w-4" />
                            </Button>
                            <Button variant="outline" size="icon" onClick={zoomIn} disabled={!imgReady}
                                title={t('dialog.image_crop.zoom_in')} aria-label={t('dialog.image_crop.zoom_in')}>
                                <ZoomIn className="h-4 w-4" />
                            </Button>
                            <Button variant="outline" size="icon" onClick={zoomOut} disabled={!imgReady}
                                title={t('dialog.image_crop.zoom_out')} aria-label={t('dialog.image_crop.zoom_out')}>
                                <ZoomOut className="h-4 w-4" />
                            </Button>
                            <Button variant="outline" size="icon" onClick={reset} disabled={!imgReady}
                                title={t('dialog.image_crop.reset')} aria-label={t('dialog.image_crop.reset')}>
                    ...
                    {noteEnabled ? (
                        <Field>
                            <FieldLabel htmlFor="image-crop-upload-note">{noteField.label}</FieldLabel>
                            <Input
                                id="image-crop-upload-note"
                                maxLength={noteMaxLength}
                                value={note}
                                onChange={e => setNote(String(e.target.value || '').slice(0, noteMaxLength))}
                    ...
                    {cropWhiteBorderEnabled ? (
                        <Field orientation="horizontal" className="h-9 w-auto">
                            <Checkbox
                                ...
                                onCheckedChange={v => setCropWhiteBorder(Boolean(v))}
                            />
                            <FieldLabel htmlFor="image-crop-white-border">{cropWhiteBorderField.label}</FieldLabel>
                        </Field>
                    ) : null}
                </div>
                ...
                <DialogFooter>
                    ...
                    <Button disabled={isConfirming || !file} onClick={() => { confirmCrop(); }}>
