...
pub fn resize_image_to_fit_limits_base64(base64data: &str) -> Result<String, AppError> {
    let raw = B64
        .decode(base64data)
...
pub fn resize_print_image_bytes(base64data: &str) -> Result<Vec<u8>, AppError> {
...
pub fn crop_print_base64(base64data: &str) -> Result<String, AppError> {
    let raw = B64
        .decode(base64data)
        .map_err(|e| AppError::Custom(format!("base64 decode: {e}")))?;
    let img =
...
pub fn crop_all_prints(ugc_folder_path: &str) -> Result<(), AppError> {
    ...
    for entry in walkdir::WalkDir::new(&folder) {
        ...
        {
            crop_print_file(p).map_err(|e| AppError::Custom(format!("{}: {e}", p.display())))?;
        }
    }
    Ok(())
}

pub fn crop_print_file(path: &Path) -> Result<bool, AppError> {
    let img = image::open(path).map_err(|e| AppError::Custom(e.to_string()))?;
    if img.width() != PRINT_CANVAS_WIDTH || img.height() != PRINT_CANVAS_HEIGHT {
        return Ok(false);
    }
    let cropped = img.crop_imm(
...

    {
        ...
        let mut old_png = png_mod::PngFile::open_read(&old_path_str).map_err(AppError::Custom)?;
        let text_chunks = old_png.get_chunks_of_type(&ChunkType::ITXT);
        if !text_chunks.is_empty() {
            let temp_str = temp_path.to_string_lossy();
            let mut new_png = png_mod::PngFile::open_rw(&temp_str).map_err(AppError::Custom)?;
...
mod tests {
    ...
    use crate::domain::png as png_mod;
    use crate::error::AppError;
    ...
    fn encode_test_png(width: u32, height: u32) -> Result<String, AppError> {
    ...
    fn decoded_dimensions(bytes: &[u8]) -> Result<(u32, u32), AppError> {
    ...
    #[test]
    fn resize_image_to_fit_limits_base64_returns_png_base64() -> Result<(), AppError> {
    ...
    #[test]
    fn resize_upload_image_bytes_pads_matching_dimensions_to_square() -> Result<(), AppError> {
    ...
    #[test]
    fn resize_print_image_bytes_outputs_print_canvas() -> Result<(), AppError> {
    ...
    #[test]
    fn crop_print_base64_crops_only_2048x1440() -> Result<(), AppError> {
    ...
    #[test]
    fn crop_print_file_preserves_itxt_chunks() -> Result<(), AppError> {
        ...
        let mut png = png_mod::PngFile::open_read(&path_str)
            .map_err(|e| AppError::Custom(format!("png read: {e}")))?;
        let metadata = png_mod::read_text_chunk("Description", &mut png, false)
            .ok_or_else(|| AppError::Custom("missing png metadata".into()))?;
